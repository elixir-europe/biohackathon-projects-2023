...
![Species used to test annotation pipelines. Clockwise from top left: Citrullus colocynthis, Drosophila melanogaster, Helleia helle, Melampus jaumei, Phakellia ventilabrum, Pocillopora cf. efusa and Trifolium dubium](fig1.png){ width=1000px }
...
![Structure of annotation workflows tested. Shown are the input data, software and pipeline structure of the UNIL (top-left), CNAG (top-right), ANNOTATO (bottom-left) and Braker3 (bottom-right) workflows](fig2.png){ width=1000px }
...
![ Number of genes annotated per genome and pipeline. Bars are coloured by workflow and grouped by species](no_genes.png){ width=1000px }
...
![OMArk completeness scores for each annotation performed on each species genome. For each protein sequence in the OMAmer database of the respective lineage, the sequence is marked as Missing (red) or Complete either once (Single - light blue) or multiple times (Duplicated - dark blue) based on its presence in the protein sequences of the given annotation. Note: As only chromosome 19 of the human genome was used, y-scale is different.](omark_complete.png){ width=1000px }

![Correlation between completeness scores computed by BUSCO (x-axis) and OMArk (y-axis). Pearson’s correlation coefficient is shown.](busco_omark.png){ width=1000px }
...
![SnpEff in HelHell1.1 - Number of effects annotated in the SNVs by region ](fig_9.png){ width=1000px }
...
![SNV counts across ilHelHell1.1 chromosome 21. The analysis pinpoints a terminal area poor in genetic variants compared with the rest of the chromosome](fig_10.png){ width=1000px }
